...
const getEmployeeById = async(req,res)=>{
    try{
        ...
        if(!employee) res.status(HTTP_STATUS.NOT_FOUND).json({ message : "Employee not found" });
...
const addNewEmployee = async (req,res)=>{
    try{
        ...
        if(existingEmployee){
            return res.status(HTTP_STATUS.CONFLICT).json({ message : "Employee Alreafy Exists"});
        }
       ...
       return res.status(HTTP_STATUS.CREATED).json({ message : "Successfuly added !"});
    }catch(error){
        return res.status(HTTP_STATUS.SERVER_ERROR).json({ message : "Internal Server Error !", "error " : error.message})
    }
}
...
const updateEmployeeById = async(req,res)=>{
    try{
        ...
        return res.status(HTTP_STATUS.OK).json({ message : "Successfuly updated !"});
    }catch(error){
        return res.status(HTTP_STATUS.SERVER_ERROR).json({ message : "Server Error !"});
    }
}
